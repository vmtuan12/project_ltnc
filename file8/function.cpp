...
bool soundFunc::loadSound()
{
    ...
    col_e_roc = Mix_LoadWAV("sound/col_e_roc.wav");
    item_pick_up = Mix_LoadWAV("sound/item.wav");

    if(player_roc == NULL || e_dis == NULL || col_e == NULL || col_e_roc == NULL || item_pick_up == NULL) return false;
    return true;
}
