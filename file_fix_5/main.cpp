...
int main(int argc, char* argv[])
{
    ...
    bool load_img = imageFunc::load_img_menuPhase(renderer);
    if(load_img == false){
    ...
    while(true)
    {
        ...
        if(playGame){
            //code here
            bool load_ingameIMG = imageFunc::load_img_bgr(renderer);
            if(load_ingameIMG == false){
                cout << "false load ingame IMG" << endl;
                imageFunc::del_img_bgr();
                imageFunc::del_img_menuPhase();
                func::del_sound_text();
                //if(gFont == NULL) std::cout << "1" << std::endl;
            ...
            while(playGame){
                ...
                if(!nameText.loadFromRenderedText(name,{51,255,153},nameFont,renderer))              //fixing
                {
                ...
                int name_x = SCREEN_WIDTH/2 - nameText.getWidth()/2;                    //nhap ten
            ...
            if(score >= highScore){
                ...
                bool conf = false;
                int playSound_count = 0;
                while(true){
                    func::textColorChange(red,green,blue);
                    if(!nameText.loadFromRenderedText(name,{red,green,blue},nameFont,renderer))              //fixing
                    {
                        cout << "Failed to render text name texture!\n";
                        imageFunc::del_img_bgr();
                        imageFunc::del_img_menuPhase();
                    ...
                    imageFunc::congratRen(renderer,pos_y_naming,nameEvent,conf,button);
                    int name_x = SCREEN_WIDTH/2 - nameText.getWidth()/2;                    //nhap ten
                    if(pos_y_naming == 0){
                        nameText.renderText(name_x,250,NULL,0.0,NULL,SDL_FLIP_NONE,renderer);
                        if(playSound_count == 0) Mix_PlayChannel(-1,wow,0);
                        playSound_count = 1;
                    }
                    SDL_RenderPresent(renderer);
                    if(conf) break;
        ...
        func::textColorChange(red,green,blue);
        if(!highScoreName.loadFromRenderedText(lordName,{red-30,green+50,blue-30},nameFont,renderer))              //fixing
        {
            cout << "Failed to render text texture!\n";
            imageFunc::del_img_menuPhase();
            func::del_sound_text();
        ...
        highScoreName.renderText(683-highScoreName.getWidth()/2,120,NULL,0.0,NULL,SDL_FLIP_NONE,renderer);
        highScoreInt.renderText(683-highScoreInt.getWidth()/2,200,NULL,0.0,NULL,SDL_FLIP_NONE,renderer);
...
bool func::loadSoundText()
{
    ...
    letsgo = Mix_LoadWAV("sound/letsgo.wav");
    wow = Mix_LoadWAV("sound/wow.wav");
    if(gFont == NULL || item_picked == NULL || fire == NULL || explode == NULL ||
       e_dis == NULL || button == NULL || letsgo == NULL || wow == NULL)
        return false;
